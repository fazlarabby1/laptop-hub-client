...
const ProductSummaryCard = ({ product }) => {
    console.log(product)
    const { brand, image, location, originalPrice, postedTime, price, productName, sellerName, usedTime } = product;
    return (
        <div className="rounded-md shadow-lg border">
            <div className="flex justify-between p-3">
                <h2 className="text-sm font-semibold leading-none mb-2">Seller Name: <span className='text-violet-700 font-bold'>{sellerName}</span></h2>
